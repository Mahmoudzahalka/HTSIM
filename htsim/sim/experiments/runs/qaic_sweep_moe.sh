#!/usr/bin/env bash
# qaic_sweep_moe.sh — test the QA-hysteresis fix on MoE.
# -qa_inc_cooldown alpha: new cwnd = alpha*achieved + (1-alpha)*old_cwnd on quick_adapt.
...
SUMMARY=../experiments/qaic_sweep_moe_results.txt
ANALYZE=./analyze.sh
SIM_ABORT_US=400000
...
{
  echo "############################################################"
  echo "# MoE8x8B (64n) QA-hysteresis (-qa_inc_cooldown) sweep. 3007 flows = clean."
...
for a in 0 2 4 8; do
  out="$RUNDIR/moe8x8b_ft64_qac${a}.out"
  echo "===== [sweep] qa_smooth=${a} -> $out ====="
  ./htsim_uec -goal "$TRACE" -sender_cc_only -rtx_stats -qa_inc_cooldown "$a" \
  ...
  while kill -0 "$hpid" 2>/dev/null; do
    ...
    echo "  [qac=$a] finished=$cur sim_now_us=${now:-?} stable=$stable"
    if [[ "$cur" == "$prev" ]]; then stable=$((stable+1)); else stable=0; fi
    prev=$cur
    if [[ -n "${now:-}" ]] && awk -v n="$now" -v c="$SIM_ABORT_US" 'BEGIN{exit !(n>c)}'; then
      note=" *** ABORTED: runaway past ${SIM_ABORT_US}us ***"
      echo "  [qac=$a]$note"; kill "$hpid" 2>/dev/null; sleep 3; kill -9 "$hpid" 2>/dev/null; break
    fi
    if [[ $stable -ge 3 && $cur -gt 0 ]]; then
      echo "  [qac=$a] complete (finished=$cur)"; kill "$hpid" 2>/dev/null; sleep 3; kill -9 "$hpid" 2>/dev/null; break
    fi
  done
  ...
  rtt=$(grep "\[RTXSTATS\]" "$out" | grep -oE "avg_rtt=[0-9.]+" | awk -F= '$2>0{s+=$2;n++} END{if(n)printf "%.1f",s/n; else print "NA"}')
  {
    echo
    echo "============================================================"
    echo "qa_inc_cooldown periods = ${a}${note}"
