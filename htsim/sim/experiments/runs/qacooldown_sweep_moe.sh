#!/usr/bin/env bash
# qacooldown_sweep_moe.sh — test the QA-hysteresis fix on MoE.
# -qa_cooldown alpha: new cwnd = alpha*achieved + (1-alpha)*old_cwnd on quick_adapt.
...
SUMMARY=../experiments/qacooldown_sweep_moe_results.txt
ANALYZE=./analyze.sh
SIM_ABORT_US=400000
...
{
  echo "############################################################"
  echo "# MoE8x8B (64n) QA-hysteresis (-qa_cooldown) sweep. 3007 flows = clean."
...
for a in 1.0 2 4; do
  ...
  ./htsim_uec -goal "$TRACE" -sender_cc_only -rtx_stats -qa_cooldown "$a" \
  ...
  rtt=$(grep "\[RTXSTATS\]" "$out" | grep -oE "avg_rtt=[0-9.]+" | awk -F= '$2>0{s+=$2;n++} END{if(n)printf "%.1f",s/n; else print "NA"}')
  {
    echo
    echo "============================================================"
    echo "qa_cooldown periods = ${a}${note}"
