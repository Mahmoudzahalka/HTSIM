...
# External watchdog + SIGTERM ends htsim cleanly the moment all Connections-M
# flows finish (no need to pre-estimate makespan).
#
# CONCURRENT-GLOBAL A2A variant, adapted to the /csl (Technion) box:
#   - matrices generated with gen_serialn_alltoall.py, parallel = N-1
#     => every source launches all (N-1) peer flows at once (Triggers 0).
#   - single filesystem (/csl, ~2.5TB free): work/ + logs/ live under OUT,
#     no separate Data partition (unlike the mahmoud_murad_allaah box).
set -u
DC=/csl/mahmoud.za/HTSIM/htsim/sim/datacenter
OUT="$DC/experiments/runs/uet/a2a_concurrent_128_1024"
DATA="${DATA:-$OUT}"                            # scratch + logs stay under OUT here
HTSIM="$DC/../build/datacenter/htsim_uec"
PARSE="$DC/../build/parse_output"
matrix="$1"; fabric="$2"

TIMEOUT="${TIMEOUT:-86400}"        # hard wall-clock cap per run (safety)
END_MS="${END_MS:-600000}"         # 600 s simulated cap (watchdog SIGTERMs first
                                   # at true completion; generous so nothing clips)
...
log="$DATA/logs/${tag}.log"           # filtered log
row="$OUT/rows/${tag}.row"            # tiny -> repo
util_row="$OUT/util_rows/${tag}.urow" # tiny -> repo
work="$DATA/work/${tag}"              # scratch (util.bin + raw stdout)
...
# --- FCT stats (regex accepts scientific notation for sim time > ~1s) ---
tmp=$(mktemp)
