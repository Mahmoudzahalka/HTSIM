...
(
  cd "$work" || exit 1
  exec timeout "$TIMEOUT" "$DC/htsim_uec" \
...
if [ -s "$work/util.bin" ] && [ -s "$work/idmap.txt" ] && [ "$status" != "failed" ]; then
  util_csv=$(python3 "$OUT/extract_util.py" "$DC/../parse_output" "$work/util.bin" "$work/idmap.txt" "$mk" 2>/dev/null || echo "")
  if [ -n "$util_csv" ]; then
